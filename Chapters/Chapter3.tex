...
\\
More formally, we use a learning rate $a$ to weight the contributions.
\[ B_t=aI_t + (1 -a)B_{t-1}\]
\\Notice that if $a=0 \Rightarrow$ we have background subtraction (so no update), while if we have $a=1 \Rightarrow$ we obtain frame differencing.
...
This is because there are changes that appear and disappear faster than the update rate, such as tree leaves, rain, snow, water of a fountain, etc.
It can be that the same pixel represents at different time' instants tree leaves and sky/building behind.
\\
Unlike single Gaussian models, MoG represents the background as a combination of multiple Gaussians (K Gaussians), allowing better representation of complex scenes. 
Each Gaussian component encapsulates information about both foreground and background elements, with weights $w_[i,t] = $, means $\mu_{i,t} = $, and variances $\Sigma_{i,t} = $ dynamically adjusted with each new frame.
\\
Practically, the theoretical complexity is simplified by assuming independence among components and uniform variance across dimensions, leading to a diagonal covariance matrix. 
This simplification enables efficient computation while retaining the essence of the model.
\\
...
\\
An essential aspect of MoG is the dynamic adaptation of weights, crucial for accurately tracking changes in the scene over time. 
The update process incorporates a learning rate to balance the influence of new observations with existing model parameters.
\\
In determining the association of Gaussians to either background or foreground, weighting factors  are evaluated against a threshold (TT). 
Gaussians contributing significantly to the accumulated weight  are considered part of the background model.
\\
