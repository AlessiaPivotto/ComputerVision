...
\section{Occlusion}
\textit{Yes, it is the same occlusion we have seen in the previous chapters.}\\
...
The typical combination of feature detection and feature tracking takes as input the \textit{Good features to track} algorithm for the detection which is coupled to another algorithm for the tracking, such as the Lucas-Kanade optical flow.
...
The idea is to replace the single estimation that we are doing with KF and EFK with multiple rappresentations: we are providing alternative solutions to our problem. 
